...
**Last updated:** 2026-07-05 (Frontend cutover Stage 1 + Wave A + Wave B complete — all `src/lib/db/*.js` modules and auth now call Neon)

- **Frontend cutover (Supabase → Neon), Stage 1 + Wave A + Wave B — committed (2026-07-05):** Phase B1/B2/B5 built the parallel Neon API layer, but the app itself still called Supabase directly for everything, including auth — so none of that new layer could be exercised through the real UI. Built the frontend cutover in staged, verified waves (parallel background build agents per module, same pattern as the backend rollout), preserving every function's exact exported signature/return shape so no `src/modules/*.jsx` UI component needed to change:
  ...
  - **Wave B (9 modules):** `importLog`+`budgetStatus`, `taxBrackets`+`income`, `budgetLineItems`, `forecastLineItems`+`forecastOverrides`, `creditCards`, `bills` — same pattern. Commits `8290ebf`, `b9ee274`, `6262505`, `c557159`, `b424d8f`, `9105548`.
  - **This completes the frontend cutover for all 17 `src/lib/db/*.js` modules** — every one now talks to Neon instead of Supabase. `npm run build` verified clean end-to-end after all waves landed (all ~50 API routes + pages compiled).
  - **Known gaps, tracked, still on Supabase (no matching Neon route yet):** `budgetCategories.js#seedDefaultCategories`, `scenarios.js#deleteAdjustment` (the built DELETE route needs a parent scenario id the real call site never passes), `scenarios.js#cloneScenario` (no atomic multi-step route). None of these are on the hot path for a fresh sign-up.
  - **Not yet done:** `src/modules/dashboard/Dashboard.jsx` and `src/modules/creditcards/CreditCards.jsx` bypass the `db/` layer entirely and still query Supabase directly — need folding onto the Neon routes. The bridge-user-id reconciliation (migrated data's `user_id` still points at a temporary bridge row, `157a1267-6adf-4371-bd72-2e9bdbca64ad`, from Phase B0 — needs a real Neon Auth sign-up through a real browser, then a one-time SQL reassignment, since this sandbox can't reach the Neon Auth domain). Live browser verification of the full cutover. The actual Phase C steps (rollback script, write freeze, final `transactions` backfill via `pg_dump`, Vercel env var flip, smoke test) all still gated on the above.
