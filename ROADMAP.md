...
**Last updated:** 2026-07-05 (Cutover to Vercel/Neon production complete; Phase D cleanup done; now in burn-in period before pausing Supabase/GitHub Pages)
...
- **Burn-in period (started 2026-07-05):** deliberately pausing Supabase and retiring the GitHub Pages fallback (`.github/workflows/deploy.yml`, still deploying the old Vite+Supabase app on every push to `main`, untouched) for a few days of real daily use on Vercel/Neon first. Next, once trusted: pause the Supabase project (reversible) and replace the GitHub Pages deploy with a redirect to `ai-capital-planning.vercel.app` — both explicitly deferred, not forgotten.
